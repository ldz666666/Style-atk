...
class Attacker:
    ...
    def Attack(self,target_model,name='',minimize=False):
        ...
        for i in range(8):
            now_path=os.path.join(self.img_dir,'noise_{}'.format(i))
        ...
        for noise_level in range(8):
            ...
            for ii in range(self.nb_iter):
                ...
                now_path=os.path.join(self.img_dir,'noise_{}/'.format(noise_level))
                #now_path=os.path.join(self.img_dir,'noise{}/step{}/'.format(noise_level,ii))
                if ii==9 or ii==0:
                    utils.save_image(g_temp,os.path.join(now_path,name+'noise_{}_step_{}.png'.format(noise_level,ii)),normalize=True, range=(-1, 1))
